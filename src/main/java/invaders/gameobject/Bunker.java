...
public class Bunker implements GameObject, Renderable {
    ...
    public Renderable renderClone(){
        ...
        Bunker curB = new Bunker();
        curB.position = new Vector2D(position.getX(), position.getY());
        curB.width = width;
        curB.height = height;
        curB.lives = lives;
        curB.image = new Image(this.image.getUrl());
        if (state instanceof GreenState){
            curB.state = new GreenState(curB);
        }else if(state instanceof YellowState){
            curB.state = new YellowState(curB);
        }else if(state instanceof RedState){
            curB.state = new RedState(curB);
        }
        return (Renderable) curB;
    }
}
