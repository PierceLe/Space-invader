...
public class Enemy implements GameObject, Renderable {
    ...
    public void update(GameEngine engine) {
        if(enemyProjectile.size()<3){
            ...
            for(Projectile p: pendingToDeleteEnemyProjectile){
                if(! enemyProjectile.remove(p)){
                    double x = p.getPosition().getX();
                    double y = p.getPosition().getY();
                    for(Projectile eP: enemyProjectile){
                        double x2 = eP.getPosition().getX();
                        double y2 = eP.getPosition().getY();
                        if(x == x2 && y == y2){
                            enemyProjectile.remove(eP);
                        }
                    }
    ...
    public String getEnemyType(){
        return this.projectileStrategy.getStrategyName();
    }
    @Override
    public Renderable renderClone(){
        Enemy curE = new Enemy(new Vector2D(this.position.getX(), this.position.getY()));
        if(projectileStrategy.getStrategyName().equals("Fast")){
            curE.setImage(new Image(new File("src/main/resources/fast_alien.png").toURI().toString(), 20, 20, true, true));
            curE.setProjectileImage(new Image(new File("src/main/resources/alien_shot_fast.png").toURI().toString(), 10, 10, true, true));
        }else if(projectileStrategy.getStrategyName().equals("Slow")){
            curE.setImage(new Image(new File("src/main/resources/slow_alien.png").toURI().toString(), 20, 20, true, true));
            curE.setProjectileImage(new Image(new File("src/main/resources/alien_shot_slow.png").toURI().toString(), 10, 10, true, true));
        }

        curE.lives = lives;
        curE.xVel = xVel;
        curE.projectileStrategy = projectileStrategy.clone();

        curE.enemyProjectile = (ArrayList<Projectile>) enemyProjectile.clone();
        curE.enemyProjectile.clear();

        for(Projectile p: enemyProjectile){
            curE.enemyProjectile.add((Projectile)((EnemyProjectile) p.renderClone()));
        }

        curE.pendingToDeleteEnemyProjectile = (ArrayList<Projectile>) pendingToDeleteEnemyProjectile.clone();
        curE.pendingToDeleteEnemyProjectile.clear();

        for(Projectile pToDel : this.pendingToDeleteEnemyProjectile){
            curE.pendingToDeleteEnemyProjectile.add((Projectile)((EnemyProjectile) pToDel.renderClone()));
        }
